...
import { useUpdateAssessment, useModules, useCourse } from '@/lib/hooks/useCourses';
...
export default function EditAssessmentPage() {
  ...
  const { register, handleSubmit, control, watch, setValue, reset, formState: { errors } } = useForm<{ questions: AssessmentQuestion[]; assessment_type: string } & Record<string, any>>({
    defaultValues: {
  ...
  useEffect(() => {
    if (assessmentData) {
      setSelectedType(assessmentData.assessment_type || 'quiz');
      if (assessmentData.allowed_file_types) {
        setAllowedFileTypes(assessmentData.allowed_file_types.split(',').filter(Boolean));
      }
      ...
      reset({
        ...
        module: assessmentData.module || assessmentData.live_session || '',
        max_score: assessmentData.max_score || 100,
        passing_score: assessmentData.passing_score || 60,
        duration_minutes: assessmentData.duration_minutes || 30,
        start_datetime: formatDateTime(assessmentData.start_datetime),
        end_datetime: formatDateTime(assessmentData.end_datetime),
        track_tab_switching: assessmentData.track_tab_switching ?? true,
        max_tab_switches: assessmentData.max_tab_switches || 3,
        submission_type: assessmentData.submission_type || 'file',
        max_file_size_mb: assessmentData.max_file_size_mb || 10,
        allow_late_submission: assessmentData.allow_late_submission ?? false,
        late_submission_deadline: formatDateTime(assessmentData.late_submission_deadline),
  ...
  const handleTypeChange = useCallback((type: string) => {
    ...
    // Reset questions based on type if changing
    if (type === 'quiz') {
      if (!fields.length || fields.some(f => f.type === 'long_answer')) {
        setValue('questions', [
  ...
  const onSubmit = (data: any) => {
    ...
    if (isLive && data.module) {
      payload.live_session = data.module;
      delete payload.module;
    }

    updateAssessment(
      { courseId, assessmentId, data: payload },
  ...
  return (
    <div className="space-y-8 pb-12">
      <div>
        <h1 className="text-3xl font-black text-(--color-text-primary)">Edit Assessment</h1>
