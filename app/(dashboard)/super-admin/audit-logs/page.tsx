...
const mockLogs = [
  {
    id: 1,
    action: "ADMIN_LOGIN",
    user: "admin@leapfrog.com",
    target: "System",
    timestamp: "2024-05-03 14:20:01",
    status: "success",
  },
  {
    id: 2,
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] mt-1 font-medium">
            Platform-wide security and activity trail
          </p>
        </div>
      ...
      <Card padding="none" className="overflow-hidden border-none shadow-2xl">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[var(--color-muted)]/30">
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">
                  Action
                </th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">
                  User
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {isLoading ? (
                ...
                <tr>
                  <td
                    colSpan={5}
                    className="px-6 py-10 text-center text-[var(--color-text-secondary)] font-medium"
                  >
                ...
                filteredLogs.map((log: any) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-5 text-sm text-[var(--color-text-primary)] font-bold">
        ...
        <div className="p-6 border-t border-[var(--color-border)] bg-[var(--color-bg-card)] flex items-center justify-between">
          <p className="text-xs text-[var(--color-text-secondary)] font-medium">
            Showing 5 of 14,282 events
          </p>
          <div className="flex gap-2">
            <button
              className="px-4 py-2 border border-[var(--color-border)] rounded-lg text-xs font-bold disabled:opacity-50"
              disabled
            >
              Previous
            </button>
            <button className="px-4 py-2 border border-[var(--color-border)] rounded-lg text-xs font-bold hover:bg-[var(--color-muted)] transition-all">
              Next
            </button>
          </div>
        </div>
