...
import { useUsers, useCreateUser } from '@/lib/hooks/useAuth';
...
export function UserManagement() {
  ...
  const getAvailableRoles = () => {
    ...
    // Admin role can create all except admin and super_admin
    return [
      { value: 'staff', label: 'Staff' },
      { value: 'tutor', label: 'Tutor' },
      { value: 'company', label: 'Company' },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="flex items-center gap-4 p-6 bg-gradient-to-br from-white/80 to-white/40 border-white/40">
        ...
        <div className="md:col-span-2 relative">
          ...
          <input 
            ...
            className="w-full h-full min-h-[72px] bg-white/60 border border-white/80 rounded-[2rem] pl-14 pr-6 outline-none focus:ring-4 focus:ring-[var(--color-primary)]/5 focus:border-[var(--color-primary)]/30 transition-all font-medium text-[var(--color-text-primary)] backdrop-blur-sm"
          />
        </div>
      </div>

      {/* Users Table */}
      <Card className="overflow-hidden border-none shadow-2xl bg-white/60 backdrop-blur-xl rounded-[2.5rem]">
                ...
                <AnimatePresence mode="popLayout">
                  {filteredUsers?.map((u: any, idx: number) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <Badge variant="primary" className="font-black px-4 py-1.5 rounded-xl border-none">
                          {u.role.replace('_', ' ')}
                        </Badge>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col gap-1.5">
                          ...
                          {u.must_change_password && (
                            <span className="text-[10px] uppercase tracking-tighter font-black text-amber-600 px-2 py-0.5 bg-amber-50 rounded-lg w-fit border border-amber-100">
                              Setup Pending
                            </span>
                          )}
                        </div>
